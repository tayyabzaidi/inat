...
if ($__action_on) {
    $pdo->bind("desigId", $__action_on);
    $__action_on_rset = $pdo->query("SELECT * FROM employee_designations WHERE desigId=:desigId");
} else {
    die("Invalid request.");
}
?>
<form name="_edit_record_modal_form" 
      ...
      method="POST" 
      action=""
      onsubmit="_ajaxCall('_edit_record_modal_form', 'management/designations/update'); return false;"
      >

    <div class="modal-header">
        <h4 class="modal-title" id="_create_record_modal_label">Edit Designation</h4>
