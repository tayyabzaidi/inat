...
$qs['deptId'] = (isset($_POST['__action_on'])) ? $lib->sanitizeTheVar($_POST['__action_on']) : '';
...
if (trim($qs['name']) == '') {
    $_SESSION['msg'][] = "Department name can not be blank";
} else {

    $pdo->bind("name", $qs['name']);
    $pdo->bind("not_id", $qs['deptId']);
    $rSet = $pdo->query("SELECT * FROM departments WHERE name=:name AND deptId!=:not_id ;");
    if (count($rSet)) {
        $_SESSION['msg'][] = "Department name already exists";
    }
}
...
if (isset($_SESSION['msg'])) {
    ...
    $pdo->bindMore($qs);
    $uQuery = "UPDATE departments SET `name`=:name , `status`=:status WHERE `deptId`=:deptId ; ";
