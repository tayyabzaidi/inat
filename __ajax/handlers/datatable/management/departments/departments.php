...
$col[0] = 'departments.deptId';
$col[1] = 'departments.name';
$col[2] = 'departments.status';
...
if ($search_value) {
    $searchFilter = " AND  departments.name LIKE '%$search_value%'  ";
}
 ...
 * Filterd Records
 */
$recordsFiltered = $pdo->row("SELECT COUNT(departments.deptId) AS recordsFiltered FROM departments WHERE 1=1 $searchFilter ")['recordsFiltered'];

/*
 * Total Records
 */
$recordsTotal = $pdo->row("SELECT COUNT(departments.deptId) AS recordsTotal  FROM departments WHERE 1=1 ")['recordsTotal'];

/*
 * Data Table Records
 */
$dtQuery = "SELECT departments.* FROM departments WHERE 1=1 $searchFilter $order  $limit  ";
