                        ...
                        <?php for ($i = 0; $i < count($recEmpData); $i++) { ?>
                            <tr>
                                ...
                                <td class="" style="text-align: left;">
                                    ...
                                    $getStatus = $pdo->query(
                                        'SELECT s.status_name from `status` s join employee_encashment_status es on s.id=es.statusId where es.encashmentId=:encashmentId;'
                                    );
                                    $HOD = '';
                                    $HR = '';
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addClaimModalLabel">Encashment</h5>
            </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO employee_encashments (unique_id, date, employee_id,comment)
                VALUES ('$unique_id', '$date', '$employee_id','$comment')";

    $result = $pdo->query($sql);

    // Step 3: Verify if there is any row and extract status name
    if (!empty($result)) {
        $encashmentId = $pdo->query("SELECT id from employee_encashments where unique_id = '$unique_id';");
        ...
        for ($i = 0; $i < count($attachments['name']); $i++) {
            ...
            $pdf_hex = '0x' . $pdf_hex;
            $pdo->bind('encashmentId', $encashmentId[0]['id']);
            $attachment = $pdo->query("INSERT INTO attachment( attachment, foreignId,`type`) VALUES (" . $pdf_hex . ",:encashmentId,'encashment')");
